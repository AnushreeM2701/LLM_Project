...
Composition (see docs/methodology.md, auto-regenerated by this script):

- Hard tier: AIME-only. Uses ALL currently curated (Include=T) AIME
  questions across the 4 study categories, unbalanced by category
  (Probability tops out at 8 available AIME questions; the other
  categories are not trimmed to match — see limitations.md for why).
  Hendrycks Level-5 ("Hard") is excluded entirely: the prior pipeline's
  data showed AIME-Hard and Hendrycks-Hard accuracy differing by ~80
  points under identical models/prompts, meaning they were not measuring
  the same construct despite sharing a difficulty label.
- Easy/Medium tiers: Hendrycks MATH, unchanged from the prior pipeline
  (balanced to EASY_MEDIUM_PER_CATEGORY questions per category, seeded
  sampling for reproducibility).

This script is the ONLY writer of final_dataset.xlsx and of the
auto-generated composition table in docs/methodology.md — the two cannot
drift apart because one script produces both from the same in-memory
dataframe, every time it runs.
"""
