...
% TODO: this chapter needs critical interpretation more than any other --
% the argument (why the pattern looks the way it does, what it means,
% where the findings should be pushed back on) must be in the author's
% own voice. Draft once Chapter~\ref{chap:results} is finalised.

% Opening paragraph(s) go here, unlabeled (no "Overview" heading -- see
% Chapter~\ref{chap:introduction} for the convention used throughout this
% dissertation). One paragraph: what this chapter does (interprets the
% results of the three research questions, discusses validity threats,
% states conclusions and future work).
...
the most frequent error overall, making up 46 out of 136 incorrect
responses (33.8\%) in the dataset. This is more than twice as many as
the next most common category, Number Theory Error, which had 18
responses (13.2\%). This shows that counting problems -- such as over-
or under-counting, or confusing permutations with combinations -- are a
shared structural weakness for Gemini, GPT-OSS-120B, and Mistral Large.
It is not a problem unique to any one model or provider.

This shared weakness should be considered alongside the Chi-square test
result in Section~\ref{sec:rq1} ($\chi^2 = 27.95$, $p = 0.111$), which
...
counts between correct and incorrect Hard-tier answers (mean 13.68 vs.
16.17; median 8.0 vs. 12.5), while Gemini and GPT-OSS-120B do not. If a
...
Gemini and GPT-OSS-120B, since all three models faced the same 51
...
but not for Gemini. Section~\ref{sec:pipeline} explains that ToT
...
time increases by 2.50x for Gemini, 4.56x for GPT-OSS-120B, and 3.01x
...
needed (Section~\ref{sec:overall-accuracy}), and the gain is large --
15.6 percentage points for each model in this tier. GPT-OSS-120B has the
biggest relative cost increase (4.56x, close to the expected 4x). Still,
it is also the model with a significant McNemar result, which suggests
that the extra computation leads to a real benefit.

For Gemini, the case is weaker. Its Hard-tier accuracy does rise
numerically under ToT (51.0\% to 56.9\%), and its Medium-tier accuracy
actually falls slightly (100.0\% to 97.5\%). Still, neither change is
distinguishable from chance given only 10 discordant pairs
(Section~\ref{sec:overall-accuracy}). Gemini still pays a 2.50x
execution-time cost for ToT, without a demonstrated accuracy benefit to
offset it. It is worth being precise about what this non-significant
result does and does not show: it does not prove that ToT has no effect
on Gemini, only that this study's sample size cannot distinguish a real
but modest effect from no effect at all. A larger sample of discordant
pairs might yet disclose a significant difference in either direction.
...
meaningful accuracy boost that likely justifies its 3--4.5x cost. For
the third model, the cost is paid without a proven benefit, but this
could be because the study did not have enough data to detect an effect.
For any new model, this question should be tested directly, just as it
was here.
...
discussed in Section~\ref{chap:discussion} (Combinatorial Counting Error
...
minimum expected cell count of 0.265. Claims at the category level or
...
further benefit the other models. Wang et al.'s Self-Consistency
\cite{wang2023selfconsistency}, which combines independently sampled CoT
...
(mean 16.17 vs. 13.68; median 12.5 vs. 8.0). This may be because Mistral
...
($p = 0.021$) and Mistral Large ($p = 0.008$), but not for Gemini
($p = 0.754$). The improvement was mainly seen in the Hard tier, where
...
This dissertation's RQ2 was limited to a descriptive analysis, so no
formal significance test was used for the step-count-versus-correctness
comparison (Section~\ref{sec:step-count}). An appropriate next step
would be to test this relationship formally. For example, a
Mann-Whitney U test could compare the step-count distributions of
correct and incorrect responses for Mistral Large, to see if the
observed gap (Section~\ref{sec:summary-findings}) is statistically
significant rather than just descriptively interesting.
...
such as the neutral-judge validation and the RQ2 significance test, were
only described or left unfinished because of the practical limits of an
MSc dissertation, as explained in Section~\ref{sec:threats-validity}.
Even with these limits, the main conclusion remains: looking only at
